...
slug: maven-troubleshooting-open-srouce
---
...
project as quickly as builds alternating between green and red for no reason.
This is a story about unstable builds, troubleshooting, and perhaps most
importantly: My way to thank all contributors to basic software infrastructure -
the infrastructure we all use and take for granted.
...
<Banner
  ...
  height={433}
  src="/img/blog/2023-02-28/xkcd_dependency.png"
  width={650}
>
  Source: <a href="https://xkcd.com/2347/">https://xkcd.com/2347/</a>
</Banner>
...
was the culprit, causing non-deterministic outcomes. Therefore, I was surprised
when I discovered the actual reasons for the recent build failures. They were
all similar to this:
...
to build [QuestDB](https://questdb.io/). Maven complains that it cannot download
the Maven Clean plugin from the Central repository. If this were just a one-off
error, I would play it down as a network hiccup. However, we were getting the
same error multiple times a day!

## The investigation, Part I

I am no Maven expert, so I did what every developer does when troubleshooting a
rather complex piece of technology: Google It!
...
[https://issues.apache.org/jira/browse/WAGON-545](https://issues.apache.org/jira/browse/WAGON-545)

It says that Azure Pipelines kill idle connections, which does not play nicely
with the habit of Maven to pool and re-use HTTP connections. It was consistent
...
There was still one question unanswered: Why have we started getting this error
only recently? Well, I had an explanation for this too: We keep adding tests, so
they run longer. Azure Pipelines kill connections because the total test
duration exceeds the idle connection timeout. It all made perfect sense.

I also found a Maven configuration property to restrict the Time-To-Live of
connections in the pool. The Maven issue I found said:
...
TTL to 30s, announced it in company Slack and felt good about myself :)
...
>>>>>>> 6b855458442ad75da7e0a0c7cf51a0ec7cd474ee
## The investigation, Part II

Blind Googling combined with a bit of logical reasoning failed to produce
...
It looks like a typically long and tedious Java stack trace. I noticed there was
...
So it was suspicious I could not see anything Wagon-related in the exception
stack trace. At this point, I resorted to the 3rd level of troubleshooting:
Reading the source code! The Maven project consists of many components; for a
non-expert, it's not simple to grasp. Eventually, I made an interesting
discovery: Maven dependency resolver supports pluggable (network) transports.
Digging deeper, I found 2 Transport implementations that can provide HTTP
access:
...
This is the
[source code of the Wagon implementation](https://github.com/apache/maven-wagon/blob/88e8574c69d83e6a658fcd2ad10f3f80926b8840/wagon-providers/wagon-http-shared/src/main/java/org/apache/maven/wagon/shared/http/AbstractHttpClientWagon.java#L560).
It uses the [Apache HTTP Client](https://hc.apache.org/index.html) internally,
and it configures it with a retry handler. There are different retry strategies
available, and the exact details are unimportant. What is important is that the
Wagon transport uses three retries by default.
...
From class and package names it’s clear that this is the transport I saw in the
exception stack traces on the build server. It also uses the Apache HTTP Client
internally but does not configure any retrying strategy!
...
to retry and immediately reports network errors. However, why has this issue
arisen only recently despite it working correctly in the past? I opened
...
Here is the hypothesis: We invoke Maven via the
...
Hence, it uses the default version, which was recently upgraded to Maven 3.9.
And we know from the release notes that Maven 3.9 switched to the new Native
transport which does not support retrying. It is all starting to make sense.
...
passed, then a second, third, and fourth. All builds were green! Such a relief.
...
ubiquitous that finding a Java developer who wouldn’t use it at least for some
projects is impossible. Maven is a basic software infrastructure we all take for
granted. It’s not backed by a large corporation or by a VC-funded startup.
Volunteers maintain it. I felt the least I could do was report my experience to
the Maven team so they could clarify whether it was intentional behavior or a
bug. I opened a ticket
[opened a ticket](https://issues.apache.org/jira/browse/MNG-7708) and wanted to
move on.

The same evening I saw one of the Maven maintainers created a
[new issue](https://issues.apache.org/jira/browse/MRESOLVER-326) in the Maven
Resolver project and linked it with the top-level Maven project issue I created
earlier. I took it as a confirmation that the behavior was not intentional and
started to poke the Maven Resolver source code once again. Only this time with
the intention to fix it.

It turns out the fix was really easy. It was a matter of creating a
[retry handler and setting the desired retry count](https://github.com/apache/maven-resolver/pull/253/files#diff-cb82a28d1938522d3e73b0df9a33bf6f80fdb854296bb2020f86fad9c139d22d).
That is exactly what the old Wagon plugin was doing. A few lines change. Then I
was wondering how to test it. None of the
...
exercised scenarios where a connection to a remote repository fails. The
existing tests were using a
...
my surprise, the maintainers reviewed the PR less than 1 hour after submitting
it and merged it the next morning. Wow! This is the way to attract new
